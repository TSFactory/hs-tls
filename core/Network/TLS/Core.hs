  ...
  where process (Alert13 [(AlertLevel_Warning, CloseNotify)]) = tryBye ctx >> setEOF ctx >> return B.empty
        ...
        process (AppData13 x) = do
            ...
            case established of
              ...
              NotEstablished      -> throwCore $ Error_Protocol ("data at not-established", True, UnexpectedMessage, Nothing)
...
onError _ Error_EOF = -- Not really an error.
            return B.empty
onError terminate err@(Error_Protocol (reason,fatal,desc, Nothing)) =
...
keyUpdate ctx getState setState = do
    ...
    unless (level == CryptApplicationSecret) $
        throwCore $ Error_Protocol ("tried key update without application traffic secret", True, InternalError, Nothing)
