    ...
    decryptData13 mver e st
        | ct == ProtocolType_AppData = do
            ...
            case unInnerPlaintext inner of
                Left message   -> throwError $ Error_Protocol (message, True, UnexpectedMessage, Nothing)
...
decryptData ver record econtent tst = decryptOf (cstKey cst)
  where cipher     = fromJust "cipher" $ stCipher tst
        ...
        get3i s ls = maybe (throwError $ Error_Protocol ("record bad format", True, BadRecordMac, Nothing)) return $ partition3 s ls
